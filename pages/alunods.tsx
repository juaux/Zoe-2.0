...
import { useAlunos } from '../hooks/useSupabaseQuery';
...
export default function Alunos() {
  ...
  const { data: alunosList = [], isLoading: loadingLista } = useAlunos();

  const irPara = (novaTab: 'cadastro' | 'lista') => {
    router.push({ pathname: '/alunos', query: { tab: novaTab } }, undefined, { shallow: true });
  };
  ...
  const startEdit = (row: any) => {
    setForm({
      ...
      email: row.email || '', nomePai: row.nomePai || '', nomeMae: row.nomeMae || '',
      responsavel: row.responsavel || '', telefoneResponsavel: row.telefoneResponsavel || '',
      foto: null, fotoUrl: row.fotoUrl || '',
    });
    setEditId(row.id);
    setTouched({});
    setErrors([]);
  ...
  const handleDelete = async (row: any) => {
    if (!confirm(`Excluir o aluno "${row.nomeCompleto}"? Essa ação não pode ser desfeita.`)) return;
    setExcluindoId(row.id);
    const { error } = await supabase.from('Alunos').delete().eq('id', row.id);
    setExcluindoId(null);
    if (error) {
  ...
  const alunosFiltrados = (alunosList as any[]).filter(a => {
    const q = busca.trim().toLowerCase();
    if (!q) return true;
    return (a.nomeCompleto || '').toLowerCase().includes(q)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editId) {
        const { error: updErr } = await supabase.from('Alunos').update(payload).eq('id', editId);
      ...
      <div style={{ maxWidth: tab === 'lista' ? 1100 : 960, margin: '0 auto' }}>
        ...
        <div style={{ marginBottom: 20 }}>
          <h1 style={{ fontSize: 22, fontWeight: 700, fontFamily: 'Poppins, sans-serif', color: 'var(--nt-text-primary)', marginBottom: 4 }}>
            {tab === 'lista' ? 'Lista de Alunos' : editId ? 'Editar Aluno' : 'Cadastrar Aluno'}
          </h1>
          <p style={{ fontSize: 13, color: 'var(--nt-text-muted)' }}>
            {tab === 'lista' ? 'Veja, edite ou remova alunos cadastrados' : 'Preencha os dados para matricular um novo aluno'}
          </p>
        </div>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 20, borderBottom: '1px solid var(--nt-border)' }}>
          ...
          }}><FaPlus style={{ fontSize: 11 }} /> {editId ? 'Editar Aluno' : 'Cadastrar Aluno'}</button>
          <button onClick={() => irPara('lista')} type="button" style={{
            ...
            display: 'flex', alignItems: 'center', gap: 6,
          }}><FaUser style={{ fontSize: 11 }} /> Lista de Alunos</button>
        </div>
          ...
          <div>
            ...
            {loadingLista ? (
              <div style={{ textAlign: 'center', padding: 60, color: 'var(--nt-text-muted)', fontSize: 13.5 }}>
                <FaSpinner style={{ animation: 'spin 0.8s linear infinite', fontSize: 20, marginBottom: 8 }} /><br />Carregando alunos...
              </div>
              ...
              <div style={{ background: 'var(--nt-surface)', borderRadius: 12, border: '1px solid var(--nt-border)', overflow: 'hidden' }}>
                <div style={{ display: 'grid', gridTemplateColumns: '52px 2fr 1fr 1fr 90px', gap: 12, padding: '12px 18px', fontSize: 11, fontWeight: 700, color: 'var(--nt-text-muted)', textTransform: 'uppercase', letterSpacing: '0.05em', borderBottom: '1px solid var(--nt-border)' }}>
                  <span></span><span>Nome</span><span>Turma</span><span>Matrícula</span><span style={{ textAlign: 'right' }}>Ações</span>
                </div>
                {alunosFiltrados.map((a: any) => (
                  <div key={a.id} style={{ display: 'grid', gridTemplateColumns: '52px 2fr 1fr 1fr 90px', gap: 12, padding: '10px 18px', alignItems: 'center', borderBottom: '1px solid var(--nt-border)', fontSize: 13.5 }}>
                    ...
                    <span style={{ fontWeight: 600, color: 'var(--nt-text-primary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{a.nomeCompleto}</span>
                    <span style={{ color: 'var(--nt-text-secondary)' }}>{a.turma || '—'}</span>
                    <span style={{ color: 'var(--nt-text-secondary)' }}>{a.matricula || '—'}</span>
                    <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
                      <button onClick={() => startEdit(a)} title="Editar" type="button" style={{ width: 30, height: 30, borderRadius: 7, border: '1px solid var(--nt-border)', background: 'var(--nt-bg)', color: 'var(--nt-text-secondary)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        <FaEdit style={{ fontSize: 12 }} />
                      </button>
                      <button onClick={() => handleDelete(a)} disabled={excluindoId === a.id} title="Excluir" type="button" style={{ width: 30, height: 30, borderRadius: 7, border: '1px solid var(--nt-danger)', background: 'transparent', color: 'var(--nt-danger)', cursor: excluindoId === a.id ? 'wait' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        {excluindoId === a.id ? <FaSpinner style={{ fontSize: 11, animation: 'spin 0.8s linear infinite' }} /> : <FaTrash style={{ fontSize: 11 }} />}
                      </button>
                    </div>
                  </div>
                ))}
