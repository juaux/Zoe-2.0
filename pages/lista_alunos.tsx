...
// Cliente Supabase importado do arquivo centralizado
...
const ListarAlunos = () => {
  ...
  const determinarTurma = (idade: string): string => {
    const idadeNum = parseInt(idade, 10);
    if (idadeNum >= 5 && idadeNum <= 8) return 'Turma A';
    if (idadeNum >= 9 && idadeNum <= 12) return 'Turma B';
    if (idadeNum >= 13 && idadeNum <= 17) return 'Turma C';
    if (idadeNum >= 18) return 'Turma D';
    return '';
  };
  ...
  const uploadFoto = async (file: File): Promise<string> => {
    try {
      ...
      const filePath = `alunos-fotos/${fileName}`;
    ...
    try {
      ...
      setAlunos(alunos.map(aluno => 
        aluno.id === selectedAluno.id ? { 
          ...aluno, 
          ...dadosParaAtualizar,
          fotoUrl: dadosParaAtualizar.fotoUrl || aluno.fotoUrl
        } : aluno
      ));
      ...
      await Swal.fire({
        title: 'Erro!',
        text: error instanceof Error ? error.message : 'Erro ao atualizar aluno',
        icon: 'error',
        confirmButtonText: 'OK',
      });
    } finally {
  ...
  const fetchAlunos = async () => {
    try {
      ...
      const normalizados = (data || []).map(aluno => ({
        ...aluno,
        fotoUrl: aluno.fotoUrl || aluno.foto,
        foto: aluno.foto || aluno.fotoUrl,
        sexo: aluno.sexo === "Masculino" || aluno.sexo === "Feminino" ? aluno.sexo : null
      }));

      setAlunos(normalizados);
      console.log('Alunos carregados:', normalizados.length);
    } catch (error) {
      console.error('Erro ao buscar alunos:', error);
      const errorMessage = error instanceof Error ? error.message : 'Erro desconhecido';
      Swal.fire('Erro', `Não foi possível carregar os alunos: ${errorMessage}`, 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id: number) => {
    const result = await Swal.fire({
      title: 'Tem certeza?',
      text: 'Você está prestes a deletar este aluno. Essa ação não pode ser desfeita!',
      icon: 'warning',
    ...
    try {
      ...
      await Swal.fire('Deletado!', 'O aluno foi deletado com sucesso.', 'success');
    } catch (error) {
      console.error('Erro ao deletar o aluno:', error);
      await Swal.fire(
        'Erro!',
        'Ocorreu um erro ao deletar o aluno: ' + (error instanceof Error ? error.message : 'Erro desconhecido'),
        'error'
      );
    }
  };
  ...
  const handleShowCard = (aluno: Aluno) => {
    setSelectedAluno({
      ...aluno,
      fotoUrl: aluno.fotoUrl || aluno.foto,
    });
  ...
  const handleShowPdf = (aluno: Aluno) => {
    setSelectedAluno({
      ...aluno,
      fotoUrl: aluno.fotoUrl || aluno.foto,
    });
  ...
  <Table size="small">
    <TableHead>
      <TableRow>
        ...
        <TableCell sx={{ fontWeight: 'bold', fontSize: '0.75rem', py: 0.5, width: '60px', textAlign: 'center' }}>Turma</TableCell>
    ...
    <TableBody>
      {alunos
        .filter(aluno => searchTerm === '' || 
          String(aluno[searchField as keyof Aluno] || '').toLowerCase().includes(searchTerm.toLowerCase()))
        .map(aluno => (
          <React.Fragment key={aluno.id}>
            <TableRow hover>
              <TableCell sx={{ fontSize: '0.75rem', py: 0.5 }}>{aluno.matricula}</TableCell>
              <TableCell sx={{ py: 0.5 }}>
                <Avatar 
                  src={aluno.fotoUrl || aluno.foto} 
                  sx={{ width: 32, height: 32, mx: 'auto' }}
                />
              </TableCell>
              <TableCell sx={{ fontSize: '0.75rem', py: 0.5 }}>{aluno.nomeCompleto}</TableCell>
              <TableCell sx={{ fontSize: '0.75rem', py: 0.5, textAlign: 'center' }}>{aluno.idade}</TableCell>
              <TableCell sx={{ fontSize: '0.75rem', py: 0.5 }}>{aluno.cursos}</TableCell>
              <TableCell sx={{ fontSize: '0.75rem', py: 0.5, textAlign: 'center' }}>{aluno.turma?.replace('Turma ', '')}</TableCell>
              ...
              <TableCell sx={{ py: 0.5, textAlign: 'center' }}>
                <Box sx={{ display: 'flex', justifyContent: 'center', gap: 0.5 }}>
                  ...
                  <IconButton size="small" onClick={() => handleEdit(aluno)} sx={{ p: 0.5 }}>
                    <Edit fontSize="small" />
                  </IconButton>
                  <IconButton size="small" onClick={() => handleDelete(aluno.id)} sx={{ p: 0.5 }}>
                    <Delete fontSize="small" />
                  </IconButton>
                  <IconButton size="small" onClick={() => handleShowCard(aluno)} sx={{ p: 0.5 }}>
                    <CreditCard fontSize="small" />
                  </IconButton>
                  <IconButton size="small" onClick={() => handleShowPdf(aluno)} sx={{ p: 0.5 }}>
                  ...
                  <Box sx={{ 
                    ...
                    <Grid container spacing={1}>
                      <Grid item xs={12} md={4}>
                        ...
                        <Typography sx={{ fontSize: '0.75rem' }}>
                          <strong>Data Inscrição:</strong> {aluno.dataAtual}
                        </Typography>
                        <Typography sx={{ fontSize: '0.75rem' }}>
                          <strong>Data Nasc.:</strong> {new Date(aluno.dataNascimento).toLocaleDateString("pt-BR")}
                        </Typography>
                      </Grid>
                      <Grid item xs={12} md={4}>
                        <Typography sx={{ fontSize: '0.75rem' }}>
                      ...
                      <Grid item xs={12} md={4}>
                        <Typography sx={{ fontSize: '0.75rem' }}>
                          <strong>Endereço:</strong> {aluno.endereco || 'Não informado'}
                        </Typography>
                        <Typography sx={{ fontSize: '0.75rem' }}>
                          <strong>Bairro:</strong> {aluno.bairro || 'Não informado'}
                        </Typography>
                        <Typography sx={{ fontSize: '0.75rem' }}>
                          <strong>Cidade:</strong> {aluno.cidade || 'Não informado'}, {aluno.uf || 'UF'}
                        </Typography>
                        <Typography sx={{ fontSize: '0.75rem' }}>
                          <strong>CEP:</strong> {aluno.cep || 'Não informado'}
                        </Typography>
                      </Grid>
                    </Grid>
                  </Box>
...
<Modal open={isModalOpen} onClose={closeModal}>
  <Box sx={{ 
    ...
    boxShadow: 24, 
    p: 2,
    fontSize: '0.875rem' // Tamanho de fonte reduzido para todo o modal
  }}>
  ...
  <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50 print:bg-transparent">
    ...
    >
      ...
      <div id="card-content" className="w-full h-full flex flex-col relative">
        ...
        <div className="flex-1 bg-stone-100 m-2 rounded-lg flex flex-col overflow-hidden">
          {/* Logo in top left corner */}
          <div className="absolute left-4 z-20" style={{ top: "0.75rem", left: "0.75rem" }}>
            <img
              src="/zoe/zoe.png" // Caminho do logo
              alt="Logo"
              className="w-6 h-8" // Ajuste o tamanho conforme necessário
            />
          </div>

          {/* Photo Area - Taking up 60% of the card */}
          <div className="flex-1 pt-2 relative flex items-center justify-center" style={{ width: "80%", height: "50%", overflow: "hidden", margin: "0 auto" }}>
  {selectedAluno.fotoUrl ? (
    <Image
      src={selectedAluno.fotoUrl}
      alt={`Foto de ${selectedAluno.nomeCompleto}`}
      ...
      <span className="text-gray-500 text-sm">Sem foto</span>
          ...
          <div className="bg-white p-2 flex justify-between items-center">
            <div className="flex flex-col">
              <h2 className="text-navy-900 text-[17px] font-bold tracking-wide">
                {selectedAluno.nomeCompleto
                  ? selectedAluno.nomeCompleto.split(' ')[0] + ' ' + selectedAluno.nomeCompleto.split(' ').pop()
                  : "FULL NAME"}
              </h2>
              <p className="text-gray-700 text-sm">
                Matrícula {selectedAluno.matricula || "Number"}
              </p>
              <p className="text-gray-700 text-sm">
                {selectedAluno.cursos || "Curso não informado"}
              </p>
              <div className="h-0.5 w-full bg-gray-300 mt-1"></div>
            </div>

            {/* QR Code */}
            <div className="w-14 h-14 bg-white border border-gray-800 p-1 flex items-center justify-center">
              {selectedAluno.qrcode ? (
                <Image src={selectedAluno.qrcode} alt="QR Code" className="w-full h-full" width={56} height={56} unoptimized />
              ) : (
                <Image
                  src={`https://api.qrserver.com/v1/create-qr-code/?size=80x80&data=${encodeURIComponent(window.location.origin + "/openModalWithId?id=" + selectedAluno.id)}`}
                  alt="QR Code" className="w-full h-full" width={56} height={56} unoptimized
                />
              )}
            </div>
          </div>
        ...
        onClick={() => {
          ...
          if (printWindow) {
            const printContent = `
              ...
              <html>
                <head>
                  <title>Cartão de Identificação - ${selectedAluno.nomeCompleto}</title>
                  <style>
                    ...
                    .logo { position: absolute; top: 8px; left: 8px; z-index: 20; }
                    .photo-area { 
                      width: 80%; height: 50%; margin: 8px auto 0; display: flex; align-items: center; justify-content: center;
                      background: #e0e0e0; border-radius: 4px;
                    }
                    .bottom-bar { background: white; padding: 8px; display: flex; justify-content: space-between; align-items: center; position: absolute; bottom: 0; left: 0; right: 0; border-top: 1px solid #ddd; }
                    .qr-code { width: 56px; height: 56px; background: white; border: 1px solid #333; padding: 4px; border-radius: 4px; }
                    .aluno-info h2 { margin: 0; font-size: 16px; font-weight: bold; color: #1a365d; line-height: 1.2; }
                    .aluno-info p { margin: 2px 0; font-size: 11px; color: #4a5568; line-height: 1.1; }
                  </style>
                </head>
                <body>
                  <div class="card-container">
                    ...
                    <div class="photo-area">
                      ${selectedAluno.fotoUrl ? 
                        `<img src="${selectedAluno.fotoUrl}" alt="Foto" style="width: 100%; height: 100%; object-fit: cover; border-radius: 4px;">` : 
                        '<span style="color: #666; font-size: 12px;">Sem foto</span>'
                      }
                    </div>
                    <div class="bottom-bar">
                      <div class="aluno-info">
                        <h2>${selectedAluno.nomeCompleto || 'Nome não informado'}</h2>
                        <p>Matrícula: ${selectedAluno.matricula || 'N/A'}</p>
                        <p>${selectedAluno.cursos || 'Curso não informado'}</p>
                      </div>
                      <div class="qr-code">
                        <img src="https://api.qrserver.com/v1/create-qr-code/?size=80x80&data=${encodeURIComponent(window.location.origin + "/openModalWithId?id=" + selectedAluno.id)}" alt="QR Code" style="width: 100%; height: 100%;">
                      </div>
                    </div>
                  </div>
                  <script>
      ...
      <button
        onClick={async () => {
          try {
            ...
            pdf.save(`cartao-identificacao-${selectedAluno.nomeCompleto || 'aluno'}.pdf`);
          } catch (error) {
    ...
    <Box id="pdf-content" sx={{ border: 1, p: 2, mb: 2 }}>
      ...
      <Box display="flex" mb={1}>
        ...
        <Box width="75%">
          ...
          <Typography variant="body2"><strong>Matrícula:</strong> {selectedAluno?.matricula}</Typography>
          <Typography variant="body2"><strong>Curso:</strong> {selectedAluno?.cursos}</Typography>
          <Typography variant="body2"><strong>Data de Nascimento:</strong> {selectedAluno?.dataNascimento}</Typography>
          <Typography variant="body2"><strong>Idade:</strong> {selectedAluno?.idade} anos</Typography>
        </Box>
      </Box>
      ...
      <Grid container spacing={1} mb={1}>
        <Grid item xs={6}>
          ...
          <Typography variant="body2"><strong>RG:</strong> {selectedAluno?.rg}</Typography>
          <Typography variant="body2"><strong>CPF:</strong> {selectedAluno?.cpf}</Typography>
        </Grid>
        <Grid item xs={6}>
          <Typography variant="subtitle2" fontWeight="bold" gutterBottom borderBottom={1}>
            Contato
          </Typography>
          <Typography variant="body2"><strong>Telefone:</strong> {selectedAluno?.telefone}</Typography>
          <Typography variant="body2"><strong>Email:</strong> {selectedAluno?.email}</Typography>
        </Grid>
      </Grid>
      ...
      <Box mb={1}>
        ...
        <Typography variant="body2">{selectedAluno?.endereco}, {selectedAluno?.bairro}</Typography>
        <Typography variant="body2">{selectedAluno?.cidade} - {selectedAluno?.uf}, CEP: {selectedAluno?.cep}</Typography>
      </Box>
      ...
      <Box>
        ...
        <Typography variant="body2"><strong>Pai:</strong> {selectedAluno?.nomePai}</Typography>
        <Typography variant="body2"><strong>Mãe:</strong> {selectedAluno?.nomeMae}</Typography>
        <Typography variant="body2"><strong>Responsável:</strong> {selectedAluno?.responsavel}</Typography>
        <Typography variant="body2"><strong>Contato do Responsável:</strong> {selectedAluno?.telefoneResponsavel}</Typography>
      </Box>
    </Box>
          ...
          onClick={async () => {
            try {
              ...
              pdf.html(document.getElementById("pdf-content"), {
                callback: (pdf) => {
                  pdf.save(`ficha-aluno-${selectedAluno?.nomeCompleto}.pdf`);
                },
                margin: [10, 10, 10, 10],
                autoPaging: "text",
                width: 190,
